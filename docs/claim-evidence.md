...
| C7 | Capability environments: an unauthorized call is unnameable, not merely denied | T | Stage-B capability smuggle test (forged CapToken refused pre-machine-re-entry, zero trace); `TestSQLQueryTypedParameterized` leg (c) ungranted parks `capability.revoked`; **Stage-F R1** `TestR1SQLInjectionFixtureFamily` (25-case adversarial family: std/sql SELECT-only is engine-enforced under a READ ONLY txn, ungranted caller `capability.revoked`, no read) |
| C8 | Deploy is a commit; rollback is a WHERE clause (as-of) | T+D | `scripts/demo-admit-rollback.sh`; `scripts/scenario-d-asof-rollback.sh` (UI-observed); HTTP `?as_of=` + `?as_of=` session mount (`internal/kernel/asof_mount_test.go`); **Stage-F R3 as-of now reconstructs historical ROW DATA, not only schema/behavior**: `TestR3AsOfRowDataReconstruction` + `TestR3AsOfPIIStaysMasked`, `scripts/scenario-d2-asof-data.sh` (exit 0) |
...
| C21 | Durable-condition restarts render as choices for an operator — or an agent | T+D | operatorPlane condition inbox (`TestOperatorPlaneListsConditionsAndRefusal`); agent-facing decisions measured by the M5 §7 gate (0.968 acc, flip executed). **Stage-F retired R-restart-flip**: R7 `TestR7AgentRestartRealParkedWorkflow` (agent `condition.restart` over the real MCP door drives a REAL parked `taak.signal` workflow to `resolved:approve`, exactly-once, under the flipped authority; refused `RESTART_DISABLED` while the §7 gate is red); R4 `TestR4OperatorPlaneReactiveWritesAndDelta` + `scripts/scenario-f-operatorplane.sh` (the operator plane's restart button is a REAL write over the same door, SSE-live) |
...
| C23 | Settings is an admission client; a tenant's custom field is a scoped overlay row; config and code are one substance | T+D | `scripts/scenario-a-field-add.sh` (tenant field-add through the gate, `--base` optimistic concurrency, old sessions resync). **Stage-F retired R-settings-form**: a point-and-click `SettingsForm` component now ships AS ADMITTED ROWS (`crm/settingsform.ts`, lowered through the same §1 gate as `AccountCard`), its `(field name, type)` walking the EXACT SAME HTTP `/admit` door — `scripts/scenario-a2-settings-form.sh` (exit 0), `SettingsForm` admitted + asserted in `TestCRMReferenceAppEndToEnd`; gate-level refusals witnessed on the form path (out-of-roster type → `tsgo` TS2322; stale base → `cas` STALE_BASE). NARROWED residue (not residue-only): submit is not yet on the reactive `/session` bus — a browser reaches `/admit` with a plain POST (why-safe in STAGE-F §9.2) |
| C24 | The workflow canvas and form designer are projectional editors over the catalog | R | **R-projectional-editors — STAYS a residue after Stage F (the operator did NOT scope it this session)**: not built in v1 (concept-doc surface, no GATE-1 §4 deliverable binds it; why-safe: derived form/table/board/dashboard prove the catalog→UI direction, and Stage-F R2's admitted `SettingsForm` proves the UI→admission direction the editor would ride; the projectional editor itself is v2). **The only residue-ONLY claim.** |
...
| C27 | std/ batteries complete for the B2B envelope, versioned with the epoch, no raw-HTML escape hatch | T+D+R | 72-entry genesis, two-fresh-DB byte-identical + 219-boundary kill sweep; `TestD3ComponentOutside25Rejected`. **Stage-F R13 promoted `std/files` + `std/i18n` DEFER → SHIP** as epoch-1 genesis rows (Rule-of-Three-scoped to `crm/attach`): `scripts/scenario-g-files-i18n.sh` (exit 0, admitted + localized attachment spooled, content-addressed handle), genesis two-fresh-DB byte-identical re-verified (root `6b958652…` → `e45baaa9…`, std pointers 75 → 79); ships as MODULES not a 14th field type (roster stays 13). NARROWED residue (why-safe in STAGE-F §9.13): `files.get`/`files.list`, an in-substrate blob table + capability-gating `files.put`, and i18n pluralization/ICU are unbuilt = unconsumed; `test.fake` remains a stub (no third consumer) |
...
**Tally (re-tallied at the Stage-F close-out, 2026-07-18): 31 claims → 30 carry a
red-path test, 18 carry a runnable demo, 4 carry a named residue (C6, C24, C27, C30 —
every one with a why-safe above; C24 is the only claim whose evidence is
residue-ONLY).**

*Re-tally provenance (nothing silent). The Stage-E gate quoted this line as "27 test /
14 demo / 6 residue"; that summary lagged its own table — the per-row Kind cells at the
Stage-E exit already summed to **29 test / 16 demo / 6 residue** (C23 and C24 are the
only two rows without a T; sixteen rows carry a D). Stage F then moved three rows:*
- *C21 `T+R → T+D` — R-restart-flip retired (R7 real-frame agent restart + R4
  operatorPlane real writes); gains the `scenario-f` demo.*
- *C23 `D+R → T+D` — R-settings-form retired (R2 `SettingsForm` ships as admitted rows);
  gains a red-path test (`TestCRMReferenceAppEndToEnd`).*
- *C27 `T+R → T+D+R` — R13 shipped `std/files`+`std/i18n`; gains the `scenario-g` demo;
  the residue survives but NARROWED (files.get/list, ICU, `test.fake` stub).*

*Net: test 29 → **30** (+1, C23), demo 16 → **18** (+2, C21/C27), residue 6 → **4**
(−2: C21, C23 leave; C6/C24/C27/C30 remain). No claim's evidence weakened; C24 stays a
residue by operator scope (projectional editors not requested this session).*
