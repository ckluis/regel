...
- Phase 4 Stage D (std/ slice + reactive layer, est. 3 sessions): IN PROGRESS — fable build sub-orchestrator.

## Next
- Stage D → gate + operator re-decision → E (proof CRM + claim-evidence, incl. the Stage-C OPEN real-LLM M5 gates).
- Stage D plugs the full erf `resource(...)` pass roster + 13 field types + ADR-11 wiring behind the EXISTING step-5a derivation seam (buildPlan → derived_resource/derived_artifact + migration_sql) — no verifier-semantics change; cfr.EncodableTags() stays the single V5 lattice source; catalog.NamePath stays the single name→path function (3 consumers). STAGE-B.md §10 outbox-dispatcher / message match-predicates / event wakes / reaper breaker remain Stage-D items per their ADRs.
