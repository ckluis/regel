...
**BUILD-E (D3): hand-authored component lowering made concrete.** Stage-D lowered
only the derived form/table/detail; the "and each hand-authored component-kind
definition" clause above is now a real admission pass (`lowerComponent`). A def
composing the tier-1 vocabulary — a single-param `(props) => <composition>` whose
leaves bind `props.<field>` — lowers to a `component_template` `derived_artifact`
(a per-DEFINITION artifact, so `requiredPasses`/V6 `DERIVE_PARITY` are untouched;
V6's eleventh-pass parity remains on the resource `template`). A `?component=<name>`
mount overlays it into the detail slot over the view's resource row, and the mount
marks a slot bound to a pii resource field masked at its §7 leaf — so a hand-authored
component renders + patches through the identical erf point-read + `RenderFirstPaint`
+ `Diff` + rowId-subscription path as a derived detail, and the six-leaf PII proof
holds (V2 rejects a pii value at any non-leaf component site at admission, before any
template emits). NAMED RESIDUE: only the `props.<field>` bind depth is rendered
(via the derived `Field` path); deeper binding expressions (`EvalSlotExpr` member
chains / index / calls) are the widening — the reference corpus needs only depth-1.
